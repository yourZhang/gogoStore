...
import com.store.system.mapper.AdminMapper;
...
public class AdminServiceImpl implements AdminService {
    ...
    public boolean login(Admin admin) {
        ...
        record.setStatus("1"); //用户状态必须是1才可用登录使用系统
        record.setLoginName(admin.getLoginName());
        //select * from tb_admin where login_name=** and status=1
        Admin admin1 = adminMapper.selectOne(record);
        if(null != admin1){
            String hashpw = admin1.getPassword();
            return BCrypt.checkpw(admin.getPassword(), hashpw);
        }
        return false;
    }
